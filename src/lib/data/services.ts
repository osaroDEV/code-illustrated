import { MdModelTraining } from "react-icons/md";
import { FaProjectDiagram } from "react-icons/fa";
import { LiaHandsHelpingSolid } from "react-icons/lia";
import { GiSkills } from "react-icons/gi";


export const services = [
    {
        id: 1,
        header: 'Structured Online Courses',
        body: "Our courses provide a curriculum covering various programming languages and technologies, often with coding exercises, and projects.",
        icon: MdModelTraining
        
},
    {
        id: 2,
        header: 'Project-Based Learning',
        body: "A significant focus is placed on practical application through projects, enabling students to build a portfolio showcasing their skills.",
        icon: FaProjectDiagram
},
    {
        id: 3,
        header: 'Live Online Classes/Mentorship',
        body: "We offer live sessions with instructors, allowing for real-time interaction, Q&A, and personalized guidance. We also provide mentoring.",
        icon: LiaHandsHelpingSolid

},
    {
        id: 4,
        header: 'Portfolio Support',
        body: "We provide support in creating a portfolio that showcases your skills and projects, helping you stand out to potential employers.",
        icon: GiSkills
},
]